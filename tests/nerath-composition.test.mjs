...
import personaInstruction from "../agent/instructions/25-persona.ts";
import nerathInstruction from "../agent/instructions/30-nerath.ts";

test("Composition: persona instruction returns non-empty content when nerathMode: true and PERSONA.md present", () => {
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-data-"));
  const tmpVaultDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-vault-"));
  const origDataDir = process.env.ASSISTANT_DATA_DIR;
  const origVaultDir = process.env.ASSISTANT_VAULT_DIR;

  try {
    process.env.ASSISTANT_DATA_DIR = tmpDataDir;
    process.env.ASSISTANT_VAULT_DIR = tmpVaultDir;

    fs.writeFileSync(path.join(tmpDataDir, "settings.json"), JSON.stringify({ nerathMode: true }));
    fs.writeFileSync(path.join(tmpVaultDir, "PERSONA.md"), "# Test Persona\nSarcastic analytical guide.");

    assert.equal(isNerathModeOn(), true, "nerathMode is on");

    const personaRes = personaInstruction.events["turn.started"]();
    assert.ok(personaRes, "persona instruction returned result");
    assert.ok(personaRes.markdown.includes("Test Persona"), "contains PERSONA.md text when nerathMode is on");
    assert.notEqual(personaRes.markdown, "", "persona markdown is non-empty");

    const nerathRes = nerathInstruction.events["turn.started"]();
    assert.ok(nerathRes.markdown.includes(NERATH_CONSTITUTION_CORE), "nerath instruction includes constitution");
    assert.ok(nerathRes.markdown.includes(NERATH_POSITIVE_LAYER), "nerath instruction includes positive layer");
  } finally {
    if (origDataDir !== undefined) process.env.ASSISTANT_DATA_DIR = origDataDir;
    else delete process.env.ASSISTANT_DATA_DIR;

    if (origVaultDir !== undefined) process.env.ASSISTANT_VAULT_DIR = origVaultDir;
    else delete process.env.ASSISTANT_VAULT_DIR;
...
test("Composition: persona instruction returns empty string gracefully when PERSONA.md missing", () => {
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-data-"));
  const tmpVaultDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-vault-empty-"));
  const origDataDir = process.env.ASSISTANT_DATA_DIR;
  const origVaultDir = process.env.ASSISTANT_VAULT_DIR;
  ...
  try {
    ...
    fs.writeFileSync(path.join(tmpDataDir, "settings.json"), JSON.stringify({ nerathMode: true }));
    // PERSONA.md intentionally omitted

    const personaRes = personaInstruction.events["turn.started"]();
    assert.equal(personaRes.markdown, "", "returns empty string when PERSONA.md is missing");
  } finally {
    if (origDataDir !== undefined) process.env.ASSISTANT_DATA_DIR = origDataDir;
    else delete process.env.ASSISTANT_DATA_DIR;
...
test("Composition: precedence line present when both persona and constitution injected", () => {
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-data-"));
  const tmpVaultDir = fs.mkdtempSync(path.join(os.tmpdir(), "nerath-comp-vault-"));
  const origDataDir = process.env.ASSISTANT_DATA_DIR;
  const origVaultDir = process.env.ASSISTANT_VAULT_DIR;
  ...
  try {
    ...
    const personaRes = personaInstruction.events["turn.started"]();
    assert.ok(
      personaRes.markdown.includes("Precedence rule") ||
        personaRes.markdown.includes("constitution register governs"),
      "precedence line present in persona markdown"
    );
  } finally {
    if (origDataDir !== undefined) process.env.ASSISTANT_DATA_DIR = origDataDir;
    else delete process.env.ASSISTANT_DATA_DIR;
...
test("Re-assert Brief 1 snapshot guard: NERATH_CONSTITUTION_CORE and NERATH_IDENTITY_BOUNDARIES unchanged", () => {
...
- Do not label the user with identity formulas without direct relevance; do not defend a working identity formula against the user; do not turn creative play into a mandatory external test; do not treat dropping a project as loss of subjecthood; do not treat low-power state as proof of personality change; do not invent continuity or rupture narratives where mood or priority simply shifted.`
      ...
      .digest("hex"),
    "IDENTITY BOUNDARIES snapshot unchanged"
  );
});
